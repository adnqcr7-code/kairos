...
## Role Definition
Improves SQL queries, indexes, joins, filters, and execution plans for speed and accuracy.
...
- Workflows involving tools or platforms such as: PostgreSQL, SQLite, MySQL.
...
## Real-World Examples
- **PostgreSQL style use case:** Apply this skill to improve a real workflow, page, system, bot, prompt, or decision.
...
## Example Prompt
"Act as the Sql Query Optimizer. I need help with [goal]. Current state: [details]. Constraints: [limits]. Produce [specific output] and include a checklist to verify it."
...
### Activation Trigger
Use **Sql Query Optimizer** when the task requires specialized judgment in **Advanced Data**, especially when the user needs one of these outputs:
...
```text
Use the Sql Query Optimizer skill.
Goal: [what I need]
