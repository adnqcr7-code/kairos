...
## Role Definition
Designs sign-up, login, session handling, roles, password reset, OAuth, and account security flows.
...
- Workflows involving tools or platforms such as: Clerk, Auth.js, Firebase Auth.
...
## Real-World Examples
- **Clerk style use case:** Apply this skill to improve a real workflow, page, system, bot, prompt, or decision.
...
## Example Prompt
"Act as the Auth Flow Builder. I need help with [goal]. Current state: [details]. Constraints: [limits]. Produce [specific output] and include a checklist to verify it."
...
### Activation Trigger
Use **Auth Flow Builder** when the task requires specialized judgment in **Web Apps Fullstack**, especially when the user needs one of these outputs:
...
```text
Use the Auth Flow Builder skill.
Goal: [what I need]
