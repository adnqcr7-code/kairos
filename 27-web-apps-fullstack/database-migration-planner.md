...
## Role Definition
Plans schema changes, migration order, backups, rollback steps, and data integrity checks.
...
- Workflows involving tools or platforms such as: Prisma, PostgreSQL, SQLite.
...
## Real-World Examples
- **Prisma style use case:** Apply this skill to improve a real workflow, page, system, bot, prompt, or decision.
...
## Example Prompt
"Act as the Database Migration Planner. I need help with [goal]. Current state: [details]. Constraints: [limits]. Produce [specific output] and include a checklist to verify it."
...
### Activation Trigger
Use **Database Migration Planner** when the task requires specialized judgment in **Web Apps Fullstack**, especially when the user needs one of these outputs:
...
```text
Use the Database Migration Planner skill.
Goal: [what I need]
