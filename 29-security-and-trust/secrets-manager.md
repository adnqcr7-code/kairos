...
## Role Definition
Protects API keys, tokens, passwords, session secrets, and environment variables from leaks.
...
- Workflows involving tools or platforms such as: GitHub, .env files, CI logs.
...
## Real-World Examples
- **GitHub style use case:** Apply this skill to improve a real workflow, page, system, bot, prompt, or decision.
...
## Example Prompt
"Act as the Secrets Manager. I need help with [goal]. Current state: [details]. Constraints: [limits]. Produce [specific output] and include a checklist to verify it."
...
### Activation Trigger
Use **Secrets Manager** when the task requires specialized judgment in **Security And Trust**, especially when the user needs one of these outputs:
