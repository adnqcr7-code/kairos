...
## Role Definition
Plans how agents should handle tasks that take minutes or hours, with checkpoints, resumability, state tracking, and progress reporting.
...
- Workflows involving tools or platforms such as: CI jobs, crawlers, research agents, batch processors.
...
## Real-World Examples
- **CI jobs style use case:** Apply this skill to improve a real workflow, page, system, bot, prompt, or decision.
...
## Example Prompt
"Act as the Long Running Task Orchestrator. I need help with [goal]. Current state: [details]. Constraints: [limits]. Produce [specific output] and include a checklist to verify it."
...
### Activation Trigger
Use **Long Running Task Orchestrator** when the task requires specialized judgment in **Agent Engineering**, especially when the user needs one of these outputs:
...
```text
Use the Long Running Task Orchestrator skill.
Goal: [what I need]
