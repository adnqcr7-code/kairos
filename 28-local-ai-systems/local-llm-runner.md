...
## Role Definition
Helps run local language models with realistic expectations about RAM, VRAM, quantization, speed, and model choice.
...
- Workflows involving tools or platforms such as: Ollama, LM Studio, llama.cpp.
...
## Real-World Examples
- **Ollama style use case:** Apply this skill to improve a real workflow, page, system, bot, prompt, or decision.
...
## Example Prompt
"Act as the Local Llm Runner. I need help with [goal]. Current state: [details]. Constraints: [limits]. Produce [specific output] and include a checklist to verify it."
...
### Activation Trigger
Use **Local Llm Runner** when the task requires specialized judgment in **Local Ai Systems**, especially when the user needs one of these outputs:
...
```text
Use the Local Llm Runner skill.
Goal: [what I need]
