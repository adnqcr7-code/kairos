...
## Role Definition
Tests whether a RAG system retrieves the right chunks and answers with evidence instead of hallucination confetti.
...
- Workflows involving tools or platforms such as: RAG evals, source checking.
...
## Real-World Examples
- **RAG evals style use case:** Apply this skill to improve a real workflow, page, system, bot, prompt, or decision.
...
## Example Prompt
"Act as the Retrieval Evaluator. I need help with [goal]. Current state: [details]. Constraints: [limits]. Produce [specific output] and include a checklist to verify it."
...
### Activation Trigger
Use **Retrieval Evaluator** when the task requires specialized judgment in **Local Ai Systems**, especially when the user needs one of these outputs:
...
```text
Use the Retrieval Evaluator skill.
Goal: [what I need]
