...
## Role Definition
Runs simple benchmarks for speed, memory, accuracy, coding ability, and instruction following.
...
- Workflows involving tools or platforms such as: local LLM tests, prompt evals.
...
## Real-World Examples
- **local LLM tests style use case:** Apply this skill to improve a real workflow, page, system, bot, prompt, or decision.
...
## Example Prompt
"Act as the Model Benchmark Runner. I need help with [goal]. Current state: [details]. Constraints: [limits]. Produce [specific output] and include a checklist to verify it."
...
### Activation Trigger
Use **Model Benchmark Runner** when the task requires specialized judgment in **Local Ai Systems**, especially when the user needs one of these outputs:
...
```text
Use the Model Benchmark Runner skill.
Goal: [what I need]
