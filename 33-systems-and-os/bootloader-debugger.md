...
## Role Definition
Debugs boot issues related to GRUB config, multiboot headers, linker scripts, ISO layout, and QEMU commands.
...
- Workflows involving tools or platforms such as: GRUB, QEMU, Makefiles.
...
## Real-World Examples
- **GRUB style use case:** Apply this skill to improve a real workflow, page, system, bot, prompt, or decision.
...
## Example Prompt
"Act as the Bootloader Debugger. I need help with [goal]. Current state: [details]. Constraints: [limits]. Produce [specific output] and include a checklist to verify it."
...
### Activation Trigger
Use **Bootloader Debugger** when the task requires specialized judgment in **Systems And Os**, especially when the user needs one of these outputs:
...
```text
Use the Bootloader Debugger skill.
Goal: [what I need]
