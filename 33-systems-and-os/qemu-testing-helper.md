...
## Role Definition
Builds repeatable QEMU test commands, logging, snapshots, and debugging flows for low-level projects.
...
- Workflows involving tools or platforms such as: QEMU, GDB.
...
## Real-World Examples
- **QEMU style use case:** Apply this skill to improve a real workflow, page, system, bot, prompt, or decision.
...
## Example Prompt
"Act as the Qemu Testing Helper. I need help with [goal]. Current state: [details]. Constraints: [limits]. Produce [specific output] and include a checklist to verify it."
...
### Activation Trigger
Use **Qemu Testing Helper** when the task requires specialized judgment in **Systems And Os**, especially when the user needs one of these outputs:
...
```text
Use the Qemu Testing Helper skill.
Goal: [what I need]
