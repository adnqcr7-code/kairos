...
## Role Definition
Plans backups and verifies restores so backups are not just decorative optimism.
...
- Workflows involving tools or platforms such as: databases, files, projects.
...
## Real-World Examples
- **databases style use case:** Apply this skill to improve a real workflow, page, system, bot, prompt, or decision.
...
## Example Prompt
"Act as the Backup Restore Tester. I need help with [goal]. Current state: [details]. Constraints: [limits]. Produce [specific output] and include a checklist to verify it."
...
### Activation Trigger
Use **Backup Restore Tester** when the task requires specialized judgment in **Deployment And Hosting**, especially when the user needs one of these outputs:
...
```text
Use the Backup Restore Tester skill.
Goal: [what I need]
