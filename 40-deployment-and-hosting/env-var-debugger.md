...
## Role Definition
Finds environment variable issues across local, CI, and production deployments.
...
- Workflows involving tools or platforms such as: .env, Vercel, Netlify.
...
## Real-World Examples
- **.env style use case:** Apply this skill to improve a real workflow, page, system, bot, prompt, or decision.
...
## Example Prompt
"Act as the Env Var Debugger. I need help with [goal]. Current state: [details]. Constraints: [limits]. Produce [specific output] and include a checklist to verify it."
...
### Activation Trigger
Use **Env Var Debugger** when the task requires specialized judgment in **Deployment And Hosting**, especially when the user needs one of these outputs:
...
```text
Use the Env Var Debugger skill.
Goal: [what I need]
